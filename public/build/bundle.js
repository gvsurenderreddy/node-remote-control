...
// all websocket communication events

module.exports = {

    // connection events
    DESKTOP_CONNECT: 'client_connect',
...
var socket = (0, _socket2.default)('http://' + _config.app.host + ':' + _config.app.port + '/browser');

socket.on('connect', function () {

    // draw the desktop
    socket.on(_actions2.default.DEKTOP_BROADCAST, function (data) {
...
canvas.addEventListener('mousemove', function (event) {
    ...
    if (Math.abs(pos.x - oldMousePos.x) > 5 || Math.abs(pos.y - oldMousePos.y) > 5) {
        oldMousePos = pos;
        console.log(pos);
...
canvas.addEventListener('mousedown', function (event) {
    var pos = getMousePosition(canvas, event);
    if (event.which == 3) socket.emit(_actions2.default.RIGHT_MOUSE_CLICK, pos);else socket.emit(_actions2.default.LEFT_MOUSE_CLICK, pos);
});
